...
/**
 * 希尔排序
 * 将要排序的一组数按某个增量d分成若干组（不是真的分组），每组中记录的下标相差d，对每组使用直接插入排序算法排序；当增量减至1时，排序完成。
...
function shellSort (arr) {
  ...
  for (let i = Math.floor(len / 2); i > 0; i = Math.floor(i / 2)) {
    // i为增量，j = i即arr[j]是某分组的第N个元素（例如增量为4，arr[4]是数组的第五个元素，即第一个分组的第二个元素）
    // 随着j值变化，arr[j]指向不同分组的第N个元素（1-N个分组的第一个元素N次，1-N个分组的第二个元素...）
    for (let j = i; j < len; j++) {
