 ...
 * 堆中每一个节点的值都必须大于等于（或小于等于）其子树中每个节点的值，分别称为大顶堆，小顶堆。
 * 完全二叉树的性质，假设父节点的索引为a，左孩子的索引为2a+1，右孩子的索引为2a+2，建堆的过程即
 * @param {Array} arr 
 */
function heapSort (arr) {
  let len = arr.length
  // n2 + 1 + n1 + n2 = len => 2n2 + n1 = len - 1
  // 假设len = 10, 又因为完全二叉树中度为1结点个数n1要么是0要么是1，那么n2 = 4, n1 = 1, n0 = 5
  // 假设len = 9, 又因为完全二叉树中度为1结点个数n1要么是0要么是1，那么n2 = 4, n1 = 0, n0 = 5
  // 所以完全二叉树中非叶子节点的最后一个元素的索引为Math.floor(len / 2) 或Math.floor(len / 2) + 1
  let startIndex = Math.floor(len / 2) + ( len % 2 === 0 ? 1 : 0)
  // 建立大顶堆，需要从每个非叶子节点开始调整，建完以后最大值已经跑到了根节点，即数组的第0个
  for (let i = startIndex; i >= 0; i--) {
    heapify(arr, i, len)
  }
  // 复用了之前建立的大顶堆，从根节点开始调整即可
  for (let i = len - 1; i > 0; i--) {
    [arr[i], arr[0]] = [arr[0], arr[i]]   // 根节点与最后一个节点交换
    // 因为根节点与最后一个节点做了交换，因此不满足大顶堆的要求，重新从根节点开始调整开始调整。
    heapify(arr, 0, i)    // 最后一个结点为当前最大值，不需要再参与比较，所以第三个参数为 i，即比较到最后一个结点前一个即可
  }
  return arr
}
